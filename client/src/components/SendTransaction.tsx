...
const SendTransaction: React.FC = () => {
  ...
  const txDetails = useSelector((state: RootState) => state.txDetails);
  const dispatch = useDispatch();
  const { handleSubmit } = useForm();

  const onSubmit = (data: any) => {
    console.log(data); // this can be used for additional logic before dispatching
    handleDispatch();
  };
  ...
  const validateForm = () => {
    ...
    // Example validation: ensure there is a recipient address and amount
    if (!txDetails?.to) {
  ...
  }, [dispatch, txDetails]); // txDetails dependency added to re-validate when details change
  ...
  const handleOpen = () => {
    dispatch(openModal());
    dispatch({
      type: Actions.PrepareTransactionDetails,
    });
  };
